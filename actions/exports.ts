...
export async function getSessionExportData(sessionId: string) {
  try {
    ...
    const data = sessionData.attendances.map((att) => ({
      ...
      "Marked At": formatDateTime(att.markedAt),
      Method: att.method,
    ...
    return {
      success: true,
      data,
      filename: `${sessionData.sabaq.name} - ${formatDate(
...
export async function getSabaqExportData(sabaqId: string) {
  try {
    ...
    if (
      !session?.user?.id ||
      !["SUPERADMIN", "ADMIN", "MANAGER"].includes(session.user.role)
    ) {
      return { success: false, error: "Unauthorized" };
    }
    ...
    const data = sabaqData.enrollments.map((enrollment) => ({
      ...
      "Enrolled At": enrollment.approvedAt
