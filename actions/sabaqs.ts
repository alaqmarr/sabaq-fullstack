  ...
  try {
    ...
    const sabaq = await prisma.sabaq.create({
      data: {
        ...
        ...(validatedData.locationId ? { location: { connect: { id: validatedData.locationId } } } : {}),
        ...(validatedData.janabId ? { janab: { connect: { id: validatedData.janabId } } } : {}),
...
export async function getSabaqs() {
  try {
    ...
    else if (['ADMIN', 'MANAGER', 'JANAB', 'ATTENDANCE_INCHARGE'].includes(role)) {
