...
export async function submitQuestion(sessionId: string, questionText: string) {
    try {
        ...
        // Validate input
        const validatedData = QuestionSchema.parse({ sessionId, question: questionText });
...
export async function answerQuestion(questionId: string, answerText: string) {
    try {
        ...
        if (!questionToCheck) return { success: false, error: 'Question not found' };

        // Verify access
        if (currentUser.role !== 'SUPERADMIN') {
             const isAssigned = await prisma.sabaqAdmin.findUnique({
                where: { sabaqId_userId: { sabaqId: questionToCheck.sabaqId, userId: currentUser.id } }
            });
            ...
            if (!isAssigned && !isJanab) {
                return { success: false, error: 'Unauthorized to answer questions for this sabaq' };
            }
        }
...
export async function deleteQuestion(questionId: string) {
    try {
        ...
        if (currentUser.role !== 'SUPERADMIN') {
            ...
            if (!isAssigned && !isJanab) {
                return { success: false, error: 'Unauthorized to delete questions for this sabaq' };
            }
        }
...
export async function getSessionQuestions(sessionId: string) {
    try {
        ...
        if (currentUser.role !== 'SUPERADMIN') {
             if (['ADMIN', 'MANAGER', 'JANAB', 'ATTENDANCE_INCHARGE'].includes(currentUser.role)) {
                const isAssigned = await prisma.sabaqAdmin.findUnique({
                    where: { sabaqId_userId: { sabaqId: session.sabaqId, userId: currentUser.id } }
                });
                const isJanab = await prisma.sabaq.findFirst({
                ...
                if (!isAssigned && !isJanab) {
                    return { success: false, error: 'Unauthorized access to this session' };
                }
             } else {
                 // Mumin check - must be enrolled
                 const isEnrolled = await prisma.enrollment.findUnique({
                     where: { sabaqId_userId: { sabaqId: session.sabaqId, userId: currentUser.id }, status: 'APPROVED' }
                 });
                 if (!isEnrolled) {
                     return { success: false, error: 'Unauthorized access to this session' };
                 }
...
export async function getQuestionStats(sessionId: string) {
    try {
        ...
        return { success: false, error: error.message || 'Failed to fetch question stats' };
    }
}
