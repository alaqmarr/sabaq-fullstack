...
export async function markAttendanceManual(sessionId: string, itsNumber: string) {
    try {
        ...
        const canMark = isSuperAdmin || 
                        ['ADMIN', 'MANAGER', 'ATTENDANCE_INCHARGE'].includes(currentUser.role as string) || 
                        isJanab;
        ...
        // Check if session is active (Superadmin can bypass)
        if (!sessionData.isActive && !isSuperAdmin) {
        ...
        if (existing) {
            return { success: false, error: 'Attendance already marked for this user' };
        }
        ...
        const { isLate, minutesLate } = calculateLateness(markedAt, sessionData.cutoffTime);
...
export async function markAttendanceLocation(
    ...
    try {
        ...
        if (!sessionData.sabaq.allowLocationAttendance) {
            return { success: false, error: 'Location-based attendance is not enabled for this sabaq' };
        }
        ...
        const distance = getDistance(
            { latitude: Number(sessionLocation.latitude), longitude: Number(sessionLocation.longitude) },
...
export async function getUserAttendanceHistory(userId?: string) {
    try {
        ...
        return { success: false, error: error.message || 'Failed to fetch attendance history' };
    }
}
...
export async function deleteAttendance(attendanceId: string) {
    try {
        ...
        return { success: false, error: error.message || 'Failed to delete attendance' };
    }
}
...
export async function getAttendanceStats(sessionId: string) {
    try {
        ...
        return { success: false, error: error.message || 'Failed to fetch attendance stats' };
    }
}
    ...
    try {
        ...
        const attendees = sessionData.sabaq.enrollments.map((enrollment) => {
            ...
            return {
                ...
                status: attendance ? (attendance.isLate ? 'LATE' : 'PRESENT') : 'ABSENT',
        ...
        return { success: false, error: error.message || 'Failed to fetch session attendance data' };
    }
}

// Bulk mark attendance
export async function bulkMarkAttendance(sessionId: string, updates: { userId: string; status: 'PRESENT' | 'LATE' | 'ABSENT' }[]) {
    try {
                    ...
                    if (existing) {
                        ...
                        attendance = await prisma.attendance.create({
                            data: {
                                sessionId,
                                userId: update.userId,
                                itsNumber: (await prisma.user.findUnique({ where: { id: update.userId } }))?.itsNumber || '',
                                markedAt,
                                markedBy: currentUser.id,
                                method: 'MANUAL_ENTRY',
                                isLate,
        ...
        return { success: false, error: error.message || 'Failed to bulk mark attendance' };
    }
}
